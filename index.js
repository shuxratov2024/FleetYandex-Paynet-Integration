...
const { YANDEX_PARK_ID, YANDEX_CLIENT_ID, YANDEX_API_KEY, PAYNET_LOGIN, PAYNET_PASSWORD } = process.env;
    ...
    try {
        ...
        res.data.driver_profiles.forEach(d => {
            ...
            if (!savedMapping[yandexId]) {
                savedMapping[yandexId] = { virtualId: nextID.toString(), name: `${d.driver_profile.last_name} ${d.driver_profile.first_name}` };
                nextID++;
            }
...
app.post('/paynet/rpc', authorize, async (req, res) => {
    ...
    const account = String(params.fields?.account || params.fields?.field_value || "").trim();
    ...
    if (method === 'GetInformation') {
        ...
        return res.json({
            jsonrpc: "2.0", id,
            result: { status: 0, timestamp: new Date().toISOString(), fields: { name: driver.name } }
        });
    }
    ...
    if (method === 'PerformTransaction') {
        ...
        if (processedTransactions.has(transactionId)) {
            return res.json({ jsonrpc: "2.0", id, error: { code: 201, message: "Tranzaksiya allaqachon mavjud" } });
        }

        const amount = Number(params.amount) / 100;
        if (amount <= 0) return res.json({ jsonrpc: "2.0", id, error: { code: 413, message: "Неверная сумма" } });

        const driver = virtualDatabase.get(account);
        if (!driver) return res.json({ jsonrpc: "2.0", id, error: { code: 302, message: "Mijoz topilmadi" } });

        try {
            const netAmount = amount - (amount * 0.045);
            await axios.post(`https://fleet-api.taxi.yandex.net/v2/parks/transactions`, {
                park_id: YANDEX_PARK_ID, contractor_profile_id: driver.yandexId,
                category_id: "70000000000000000000000000000001", amount: String(netAmount.toFixed(2)),
                currency_code: "UZS", description: `Paynet ID: ${transactionId}`
            }, { headers: { 'X-Client-ID': YANDEX_CLIENT_ID, 'X-API-Key': YANDEX_API_KEY } });

            processedTransactions.set(transactionId, { status: 1 });
            fs.writeFileSync(TRANSACTIONS_FILE, JSON.stringify(Object.fromEntries(processedTransactions)));
            
            return res.json({
                jsonrpc: "2.0", id,
                result: { providerTrnId: String(Date.now()), timestamp: new Date().toISOString(), fields: { client_id: account } }
            });
        } catch (err) { return res.json({ jsonrpc: "2.0", id, error: { code: 102, message: "Yandex error" } }); }
    }
    ...
    if (method === 'CheckTransaction') {
        ...
        return res.json({
            jsonrpc: "2.0", id,
            result: { transactionState: trn ? 1 : 2, timestamp: new Date().toISOString(), providerTrnId: String(Date.now()) }
        });
    }
